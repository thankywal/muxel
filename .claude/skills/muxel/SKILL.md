...
| A console button does nothing at all: no screen, no error | The screen threw, and the report never reached the operator. `handleCallback` acknowledges the press first so the button stops spinning, which spends the one answer Telegram allows per callback query, so the `answerCallbackQuery` in the catch is discarded. `editMessageText` also lacked `sendMessage`'s fallbacks, so unparseable HTML or an unchanged redraw threw on their own. Check the Worker log for `admin callback failed`. | `telegram/api.ts` editMessageText, `telegram/admin.ts` handleCallback catch |
| Deploy form says "problem parsing the Wrangler configuration file" | Usually not a parse problem. The form fetches `wrangler.jsonc` from `raw.githubusercontent.com` in the operator's browser on every page load, and GitHub rate limits anonymous requests per address, so a reloaded page or a shared mobile address gets `429` and the form reports the wrong cause. Check the browser console for the 429, and `curl -sI` the raw URL from elsewhere: `200` there plus `429` in the browser is the whole diagnosis. Also surfaces as "Cannot retrieve latest commit at this time" on the repo page. | `docs/DEPLOY-RECOVERY.md` |
