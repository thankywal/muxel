...
<p align="center"><em>这是把产品从头到尾真跑了一遍录下来的。客户向店铺自己的网站提问,答案来自店铺自己的
价目表,正是对话框背后那份菜单上印着的 5.50。这张画面里没有任何一页是我们的:这个
widget 只是一段放在别人网站上的 script 标签,而它对话的 Worker,也在一个不属于我们的
账户里。</em></p>

第一次使用?请先阅读[开始之前](#开始之前)。一共只要两个免费账号、大约十分钟,
没有什么要你去想,也没有什么要你填。全程都不需要 Telegram 账号。
...
| | |
| --- | --- |
| <img src="docs/media/your-own-console.webp" alt="那家店铺在 console 里的页面,显示今天回答了六条消息、十三位客户、十三项定了价的商品和五份文档。"> | <img src="docs/media/the-price-it-answered-from.webp" alt="价目表标签页,列着十三种饮品和价格,其中 Flat White 是 5.50,每一行都标着是店主自己录入的。"> |
| **你自己的 console,看得到你自己的部署。**它数出来的六条消息,就是上面那张图里的对话。没有为了截图安排任何东西:是部署真的答了,console 只是把它读回来。 | **答案的出处。**每一行都是店主自己的,要么手动录入,要么从他们上传的文档里读出来。助手只是引用它,而不是编出来。 |
| <img src="docs/media/what-it-could-not-answer.webp" alt="console 的消息页面显示同一段对话,最后是助手说自己没有营业时间的信息、正把问题转交给人,上方有一个 Take over 按钮。"> | |
| **它不知道的时候呢。**被问到周日几点开门——店里任何一份文档都没写——它就直说不知道,并把这个问题交给人。店主看得到整段对话,也可以中途接手。 | |

这四张都是同一次运行里的画面:一个只放了 console 生成的那一行 script 标签的普通网页上的
widget、一个普通 Cloudflare 账户里的部署,以及这个仓库发布出来的 console 本身。
没有一张是效果图,也没有一处是摆拍。
...
Muxel 不使用 R2 存储桶,也没有任何功能需要它。上传的文档会被读成文本、建立索引,
原件不会保留。

以前是保留的:把价目表读成一行行的数据,而不是读成被压平之后的文字,需要文件本身。
那个能力已经拿掉了,而一个没有读者的归档,只是把每一位客户的文档复制一份、却说不出
留着做什么。何况即使在 R2 自己的免费额度内,启用它也要求填写付款方式。
## 安全
