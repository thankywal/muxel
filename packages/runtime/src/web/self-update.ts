...
export async function runSelfUpdate(env: Env): Promise<UpdateOutcome> {
  ...
  try {
    ...
    // Everything except the operator's own workflow directory, which a token
    // without the workflows permission is not allowed to write.
    const files = upstreamTree.tree.filter(
      (entry) => entry.type === "blob" && !entry.path.startsWith(".github/"),
    );
    if (files.length === 0) {
